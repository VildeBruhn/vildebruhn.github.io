...
where $z_i$ is the expected trait value for population i in the time series, 
$z_i$ is the ancestral trait mean, 
$\sigma^2_{step_0}$ is the step distribution, 
$r$ describes the exponential decay in the net rate of change through time and is constrained to be $0$ or smaller, 
...
and $t_{min}$ is the time interval from the ancestral population to the oldest of the two populations $z_i$ and $z_j$.
